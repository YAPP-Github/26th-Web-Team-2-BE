...
public class Accommodation {
    ...
    public Accommodation update(UpdateAccommodationRequest request) {
        ...
        if (request.getLowestPrice() != null)
            this.lowestPrice = request.getLowestPrice();
        if (request.getCurrency() != null)
            this.currency = request.getCurrency();
        if (request.getReviewScore() != null)
            this.reviewScore = request.getReviewScore();
        if (request.getCleanlinessScore() != null)
            this.cleanlinessScore = request.getCleanlinessScore();
        if (request.getReviewSummary() != null)
            this.reviewSummary = request.getReviewSummary();
        ...
        if (request.getNearbyTransportation() != null) {
            this.nearbyTransportation = updateTransportationsList(this.nearbyTransportation,
                    request.getNearbyTransportation());
        }
     ...
     */
    private List<Attraction> updateAttractionsList(List<Attraction> existingAttractions,
            List<AttractionUpdate> updates) {
        if (updates == null)
     ...
     * 기존 데이터는 무시하고 request 데이터로 완전히 갈아끼움
     */
    private List<Transportation> updateTransportationsList(List<Transportation> existingTransportations,
            List<TransportationUpdate> updates) {
        if (updates == null)
            return existingTransportations;
                ...
                .map(existing -> {
                    ...
                    if (updateRequest != null) {
                        ...
                        return Amenity.builder()
                                ...
                                .description(updateRequest.getDescription() != null ? updateRequest.getDescription()
     ...
     * 숙소의 메모를 업데이트합니다.
     * 
     * @param memo 새로운 메모 내용 (null 또는 빈 문자열 허용)
     * @return 업데이트된 Accommodation 객체
     */
    public Accommodation updateMemo(String memo) {
        this.memo = memo;
        return this;
    }
}