...
public interface AccommodationDocs {
	...
	ResponseEntity<StandardResponse<AccommodationPageResponse>> getAccommodationByTableIdAndUserId(
			@Parameter(in = ParameterIn.QUERY, schema = @Schema(type = "integer"), description = "숙소가 포함된 테이블의 ID") @NotNull(message = "테이블 ID는 필수입니다.") @Min(value = 1, message = "테이블 ID는 1 이상이어야 합니다.") Integer tableId,
